...
################# FUNCTIONS ################
def print_board(board, time):
    global snake
    print()
    print(f"______________ time : {time} _______________")
    for line in board:
        for ele in line:
            print(f"{ele} ", end='')
        print()
...
def get_next_board(board, time):
    global commands, snake
    next_board = []
    is_end = False

    # check if it's time to rotate
    if time - 1 in commands["time"]:
        idx = commands["time"].index(time - 1)
        req_command = commands["command"][idx]
    else:
        req_command = None

    # set "board", "snake" according to given command
    if req_command == "L":
        next_board = rotate_board_L(board)
    elif req_command == "D":
        next_board = rotate_board_D(board)
    else:
        # get "next_board" using "board"
...
def solution(board):
    ...
    for time in range(1,int(1e+9)):

        board, is_end = get_next_board(board, time)
        print_board(board,time)  #TODO debug..
