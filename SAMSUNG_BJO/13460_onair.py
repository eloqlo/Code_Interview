...
def bfs(board, ry,rx,by,bx):
    ...
    for count in range(1,11):
        tmp_queue=[]
        for rpos, bpos in queue:    # t시점의 위치들 받아온다.
            for di in range(4):  # t시점의 위치에서 한 방향씩 기울여 본다.
                rt=rpos
                bt=bpos
                ...
                for _ in range(8):
                    ...
                    # update R
                    if not (board[b_t1[0]][b_t1[1]]=='#' and r_t1==bt) and \
                            (board[r_t1[0]][r_t1[1]]!='#'):
                        ...
                        if board[r_t1[0]][r_t1[1]] == 'O':
                            ...
                            for idx in range(1,9):
                                ...
                                check_b_t1 = (check_b_y, check_b_x)
                                if board[check_b_t1[0]][check_b_t1[1]]=='#':
                                    break
                                if board[check_b_t1[0]][check_b_t1[1]]=='O':
                                    finish_flag=False
                            if finish_flag:
                                return count
                            else:
